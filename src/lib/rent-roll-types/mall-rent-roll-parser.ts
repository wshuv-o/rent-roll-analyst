  ...
  for (let r = headerRow + 1; r < data.length; r++) {
    ...
    if (unitVal && dbaVal) {
      ...
      currentTenant = {
        ...
        // In narrow CSV, identity metadata columns overlap with charge columns,
        // so only read them from the first row in wide format; metadata rows will fill them later.
        leaseType: sections.isNarrow ? null : (str(cell(row, C.leaseType)) || null),
        unitType: sections.isNarrow ? null : (str(cell(row, C.unitType)) || null),
        leaseStatus: sections.isNarrow ? null : (str(cell(row, C.leaseStatus)) || null),
        percentInLieu: sections.isNarrow ? null : cell(row, C.percentInLieu),
        commencementDate: sections.isNarrow ? null : cell(row, C.commencementDate),
        openDate: null,
        originalEndDate: sections.isNarrow ? null : cell(row, C.originalEndDate),
        expireCloseDate: sections.isNarrow ? null : cell(row, C.expireCloseDate),
